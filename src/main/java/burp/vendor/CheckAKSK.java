...
public class CheckAKSK implements Base {
    ...
    public static final String[] keyWord = new String[]{"ACCESSKEYID","ACCESSKEYSECRET","ACCESS_KEY_ID","ACCESS_KEY_SECRET","AccessKey","AccessSecret","Secret","AccessKeySecret","SECRET_KEY","ACCESS_KEY",};
    public static List<String> customDict = new ArrayList<>();
    @Override
    ...
    private void checkAKSK(){
        if (!customDict.isEmpty()){
            for (String s : customDict) {
                if (baseRequestResponse.response().body().countMatches(s) >= 1){
                    baseRequestResponse.response().bodyToString().indexOf(s);
                    ...
                    list.add(auditIssue);
                    UI.updateUIData(new ListsModule(baseRequestResponse),s);
                }
            }
        }else {
